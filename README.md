...
A Python-based Telegram bot for monitoring and managing a Proxmox VE server directly from a Telegram chat.
...
- [Proxmox API Permissions](#-proxmox-api-permissions)
...
It is recommended to create two separate tokens in Proxmox:
1. **Standard token** for monitoring operations.
2. **Privileged token** for administrative operations (like power management).
...
Create a `docker-compose.yml` file in the project root (optional but recommended for easier management):
...
- [ ] Automatic notifications (e.g., node offline, high CPU)
- [ ] Web dashboard
- [ ] RBAC authentication
- [ ] Persistent logging
